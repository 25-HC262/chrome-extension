import { StreamController } from "../features/streaming/model/streamController.js";
...
class PanelController {
  ...
  getElement() {
    ...
    this.startTranslateButton = this.panelElement.querySelector(
      "#start-translate-button",
    );
    this.stopTranslateButton = this.panelElement.querySelector(
      "#stop-translate-button",
    );
  ...
  attachEventListeners() {
    this.startTranslateButton.addEventListener("click", () =>
      this.handleStartStreaming(),
    );
    this.stopTranslateButton.addEventListener("click", () =>
  ...
  async handleStartStreaming() {
    const selectedUsers = Array.from(userController.selectedUsers);

    if (selectedUsers.length === 0) {
      alert("해석할 참가자를 선택해주세요.");
      return;
    }

    // 서버 연결 확인 (새 컨트롤러의 connect 메서드 호출)
    await streamController.connect();

    selectedUsers.forEach((targetUserId) => {
      // 새 컨트롤러의 메서드 형식에 맞게 호출
      streamController.startUserStreaming(targetUserId, () => {
        const userData = videoController.userVideos.get(targetUserId);
        return userData ? userData.video : null;
      });
    });

    this.updateStreamingButtons(true);
    this.updateStatus(`${selectedUsers.length}명 해석 중...`, "streaming");
  }
  ...
  handleStopStreaming() {
    ...
    selectedUsers.forEach((targetUserId) => {
      streamController.stopUserStreaming(targetUserId);
    });

    this.updateStreamingButtons(false);
    this.updateStatus("대기 중", "idle");
  }
  ...
  updateStatus(message, statusType = "idle") {
    ...
    this.captureStatus.classList.remove(
      "status-idle",
      "status-streaming",
      "status-error",
    );
