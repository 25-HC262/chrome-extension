...
class StreamController {
  ...
  async connectToStreamingServer() {
    ...
    try {
      // const serverURL = ''
      const serverUrl = `https://streaming-server-648489943292.asia-northeast3.run.app/stream?userId=${this.userId}`;
  ...
  async startStreaming() {
    ...
    try {
      ...
      panelController.updateStatus(
        `${userController.selectedUsers.size}명의 사용자 스트리밍이 시작됨`,
        "streaming",
      );

      console.log(
        "Started streaming selected users:",
        Array.from(userController.selectedUsers),
      );
  ...
  async startUserStreaming(userId) {
    ...
    if (video.paused) {
      try {
        ...
        alert(
          "비디오 재생을 시작할 수 없습니다. (브라우저 자동 재생 정책 문제일 수 있습니다.)",
        );
        return;
      }
    }
    ...
    try {
      ...
      const { chosenMimeType, videoBitsPerSecond, recorderOptions } =
        this.getRecorderOptions();
      const { recorder, drawFrameId } = this.startRecorderAndDrawing(
        video,
        canvas,
        stream,
        recorderOptions,
  ...
  notifyServerOfStart(
    ...
    if (
      ...
      console.log(
        `Started streaming user ${userId} with MediaRecorder (${mimeType}) at ${videoBitsPerSecond}bps`,
      );
    }
  }
